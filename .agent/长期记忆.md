...
### P2P 中继路由功能
- **knownNodes**：记录曾经成功通信过的节点，作为潜在中继
- **routingTable**：维护分布式路由表，记录到各目标节点的下一跳
- **路由学习**：成功通信后向对端广播可达节点列表
- **多跳中继**：通过 `relaySend(target, path, data, ['relay1', 'relay2'])` 显式指定中继路径
- **核心方法**：
  - `relaySend(targetId, path, data, relayNodes)` - 通过中继节点发送
