...
### 测试文件
- `e2e/test.spec.ts` - 基础功能测试（8 个用例）
- `e2e/test-auto-routing.spec.ts` - 自动路由测试（5 个用例）
- `e2e/test-relay.spec.ts` - 中继功能测试（2 个用例）
- `e2e/test-call.spec.ts` - 通话功能测试（7 个用例）
- `e2e/test-file.spec.ts` - 文件传输测试（3 个用例）
- `e2e/test-debug.spec.ts` - 调试模式测试（2 个用例）
- `e2e/test-explicit-relay.spec.ts` - 显式中继测试（3 个用例）
- `e2e/test-status-codes.spec.ts` - 响应状态码测试（4 个用例）
- `e2e/test-reconnect.spec.ts` - 断线重连测试（3 个用例）
...
6. ✅ 应该支持不带数据参数的请求
7. ✅ 应该支持注册和注销处理器
8. ✅ 应该正确传递发送者的 Peer ID (from 参数)
9. ✅ getRoutingTable 返回空路由表
10. ✅ getKnownNodes 返回空列表
11. ✅ 应该记录直连节点及延迟
12. ✅ 应该支持多下一跳按延迟排序
13. ✅ 路由表应该支持多下一跳
14. ✅ send 方法应该自动路由（无需手动指定中继节点）
15. ✅ getActiveCall 初始返回 null
16. ✅ 应该监听通话状态变化
17. ✅ 应该支持注册和注销来电监听器
18. ✅ 来电监听器应该去重
19. ✅ getPeerId 应该同步返回 Peer ID
20. ✅ whenReady 应该等待连接就绪
21. ✅ destroy 应该销毁实例
22. ✅ 小文件应该直接传输
23. ✅ 大文件应该分片传输
24. ✅ 文件元信息应该正确传递
25. ✅ 调试模式应该输出日志
26. ✅ 非调试模式不应该输出调试日志
27. ✅ relaySend 不指定中继节点应该直接发送
28. ✅ relaySend 指定无效中继节点应该报错
29. ✅ 中继路径错误应该正确传播
30. ✅ 200 状态码应该正常返回数据
31. ✅ 处理器返回错误响应对象应该抛出错误
32. ✅ 请求超时应该抛出错误
33. ✅ 连接到不存在的节点应该报错
34. ✅ destroy 应该销毁实例并取消重连

### 未通过的测试用例 (1/35)
- ❌ 应该通过路由表自动转发 - 浏览器上下文销毁导致测试不稳定（非功能问题）
